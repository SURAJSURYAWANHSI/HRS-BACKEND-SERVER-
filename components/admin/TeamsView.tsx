...
export const TeamsView: React.FC<TeamsViewProps> = ({ workers, currentUser }) => {
    ...
    return (
        <>
            ...
            {/* Call Windows - Portal to body for proper layering */}
            {isInCall && ReactDOM.createPortal(
                <div className="fixed inset-0 z-[9000] bg-black/90 flex flex-col items-center justify-center text-white">
                    <div className="relative w-full max-w-4xl aspect-video bg-black rounded-lg overflow-hidden mb-6 border border-slate-700">
                        {/* Remote Video (Main) */}
                        {remoteStream && (
                            <video
                                ref={(videoElement) => {
                        ...
                        {localStream && (
                            <div className="absolute bottom-4 right-4 w-32 h-24 bg-slate-800 rounded-lg overflow-hidden border border-slate-600 shadow-lg">
                                <video
                                    ref={(videoElement) => {
                        ...
                        <div className="absolute inset-0 flex flex-col justify-between p-6 bg-gradient-to-t from-black/70 via-transparent to-black/70">
                            ...
                            <div className="flex justify-center gap-6">
                                ...
                                <button
                                    onClick={endCall}
                                    className="p-4 bg-red-600 hover:bg-red-700 rounded-full text-white shadow-xl shadow-red-600/30 transition-all hover:scale-105 active:scale-95"
                                    title="End Call"
                                >
                                    <PhoneOff size={20} />
                                </button>
            ...
            {globalIncomingCall && !isInCall && ReactDOM.createPortal(
                <div className="fixed inset-0 z-[9000] bg-black/90 flex flex-col items-center justify-center text-white">
                    <div className="relative w-full max-w-md p-8 bg-slate-800 rounded-lg shadow-2xl border border-slate-700 text-center">
                        <div className="w-24 h-24 mx-auto bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-4xl mb-6 shadow-lg shadow-blue-500/30">
                            {globalIncomingCall.callerId.charAt(0)}
                        </div>
                        <h3 className="text-2xl font-bold mb-2">{globalIncomingCall.callerId}</h3>
                        <p className="text-slate-300 text-lg mb-8">
                            {globalIncomingCall.callType === 'VIDEO' ? 'Video Call' : 'Audio Call'}
                        </p>
                        ...
                        <div className="flex items-center justify-center gap-8 pt-8">
                            <button
                                onClick={rejectCall}
                                className="p-5 bg-red-600 hover:bg-red-700 rounded-full text-white shadow-xl shadow-red-600/30 transition-all hover:scale-105 active:scale-95"
                                title="Decline"
                            >
                                <PhoneOff size={28} />
                            </button>
                            <button
                                onClick={answerCall}
                                className="p-5 bg-emerald-600 hover:bg-emerald-500 rounded-full text-white shadow-xl shadow-emerald-600/30 transition-all hover:scale-105 active:scale-95"
                                title="Accept"
                            >
                                <Phone size={28} />
                            </button>
